    ...
    def chunk(self, paper: Paper) -> list[Chunk]:
        ...
        for section in paper.latex.sections:
            if len(section.subsections) == 0:
                chunks.append(Chunk(text=f"Section: {section.title}\n{section.content}", chunk_id=section.title))
            else:
                for subsection in section.subsections:
                    chunks.append(Chunk(text=f"Section: {section.title}\nSubsection: {subsection.title}\n{subsection.content}", chunk_id=f"{section.title}_{subsection.title}"))
        return chunks
...
class AdaptiveChunker:
    def __init__(
        ...
        self.token_encoder: Callable[[str], list[int]] | None = embedding_config.token_encoder
        ...
        for chunk in primary_chunks:
            ...
            if token_count <= max_tokens:
                ...
                def split_text(text_to_split, base_id_suffix):
                    ...
                    split_point = max(filter(lambda x: x != -1, split_candidates), default=mid_point)
                    if split_point == -1:
                    ...
                    if second_half_tokens > max_tokens and self.overlap > 0:
                        ...
                        for reduced_overlap in range(self.overlap-1, -1, -1):
                            ...
                            if len(words) > reduced_overlap:
                                new_overlap_text = " ".join(words[-reduced_overlap:]) + " "
                                new_second_half = new_overlap_text + text_to_split[split_point+1:]
